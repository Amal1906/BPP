...
export class EventDetailsComponent implements OnInit {
  ...
  public columnDefs: ColDef[] = [
    ...
    { headerName: 'Broker/Agency', field: 'client', width: 150, tooltipField: 'client', maxWidth: 500 },
  ...
  handleDateChange() {
    ...
    if (this.selectedClient && this.route.snapshot.queryParams['process_type']) {
      this.getTileEventDetails(startDate, endDate, this.selectedClient, this.route.snapshot.queryParams['process_type']);
    } else {
      this.loadDashboardData();
    }
  }
  ...
  loadDashboardData(ignoreClientAndProcess: boolean = false) {
    this.isLoading = true;
    const { start, end } = this.dateRange.value;
    this.rowData = [];
    this.filteredRowData = [];
    ...
    // Pass empty client if ignoring client and process_type
    let clientParam = ignoreClientAndProcess ? '' : this.selectedClient;

    this.dataService.getProcessStatus(startDate, endDate, clientParam).subscribe({
      next: (response: any) => {
        if (Array.isArray(response)) {
          // If ignoring client/process, we filter nothing; otherwise, response may already be filtered
