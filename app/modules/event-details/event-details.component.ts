...
export class EventDetailsComponent implements OnInit {
  ...
  // public stepperFlow: Step[] = [
  //   { step: 'Start',  status: 'pending',    status_message: 'Awaiting trigger' },
  //   { step: 'Doc In', status: 'pending',    status_message: 'Waiting for input' },
  //   { step: 'Doc Out',status: 'pending',    status_message: 'Waiting for output' },
  //   { step: 'End',    status: 'pending',    status_message: 'Not finished yet' }
  // ];
  ...
  this.route.queryParams.subscribe(params => {
    ...
    if (startAsDate && endAsDate) {
      ...
      // if (successFlag) {
      //     this.stepperFlow = [
      //       { ...this.stepperFlow[0], status: 'done', status_message: 'Started successfully' },
      //       ...this.stepperFlow.slice(1)
      //     ];
      //   } else if (failedFlag) {
      //     this.stepperFlow = [
      //       { ...this.stepperFlow[0], status: 'failed', status_message: 'Trigger failed' },
      //       ...this.stepperFlow.slice(1)
      //     ];
      //  }
      ...
      if (client && processType) {
        ...
        this.getTileEventDetails(startForApi, endForApi, client, processType);
      } else {
  ...
  handleDateChange() {
    ...
    if (!start || !end) return;
    this.commonService.setDate({ start, end });
    // Reload data immediately when date changes
    this.loadDashboardData();
  }
  ...
  loadDashboardData() {
    ...
    this.dataService.getProcessStatus(startDate, endDate).subscribe({
      next: (response: any) => {
  ...
  getTileEventDetails(startDate: string, endDate: string, client: string, processType: string) {
    ...
    this.dataService.getTileEventDetails(startDate, endDate, client, processType).subscribe({
      next: (response: any) => {
        if (Array.isArray(response)) {
          this.rowData = response;
          // console.log('Tile Event Details Response:', response);
          this.filteredRowData = [...response];
        } else if (response.status === 'error') {
          this.toastr.error(response.message);
          this.rowData = [];
